...
考虑到完整的图着⾊算法⽐较复杂, 同学们可实现没有 **spill** (溢出)的简化版本中干涉图的构建，即假设寄存器总是够用的，不需要溢出到栈上。当然若你能实现完整的基于⼲涉图着色的寄存器分配算法, 那是再好不过的。第⼗⼆讲课堂讲稿以及本实验指导书的末尾给出了图着⾊寄存器分配的算法的具体流程, 同学们也可⾃⾏参考⻰书和虎书。
