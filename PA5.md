...
本次 PA Rust 版本的实验内容与 Java、Scala 版本的略有不同，请选择 Rust 框架的同学以 [这个](https://mashplant.gitbook.io/decaf-doc/pa5) 实验指导书为准。不过对于 **实验评分和实验报告** 这一节的要求还是一样的。
...
考虑到完整的图着⾊算法⽐较复杂, 同学们可实现没有 **spill**(溢出)的简化版本中干涉图的构建，即假设寄存器总是够用的，不需要溢出到栈上。当然若你能实现完整的基于⼲涉图染⾊的寄存器分配算法, 那是再好不过的。第⼗⼆讲课堂讲稿以及本实验指导书的末尾给出了图着⾊寄存器分配的算法的具体流程, 同学们也可⾃⾏参考⻰书和虎书。

要求同学们实现的寄存器分配算法能够正确⼯作, ⽣成的代码能够被 SPIM 执⾏。简化的算法⾯对复杂的输⼊可能失败, 但你的实现⾄少要保证 [S4/](https://github.com/decaf-lang/decaf-2019-TestCases/tree/master/S4) 中所有测试⽤例通过。
...
> 提示：在 Java 和 Scala 版本的框架中，寄存器分配算法的基类是 `RegAlloc`，目前已有一个 `BruteRegAlloc` 类继承于它，其中实现了基于贪心的寄存器分配算法。你需要参考 `BruteRegAlloc` 再新建一个 `RegAlloc` 的子类，例如名为 `GraphColorRegAlloc`，在其 `accept()` 方法中，使用用基于⼲涉图染⾊的全局寄存器分配算法实现对一个函数的寄存器分配。此外，你可能还需对干涉图也新建一个类，在其中实现着色算法。

## 实验评分和实验报告

实验评分分两部分：

- 正确性：80%。你提交的代码应当能够通过 [S4/](https://github.com/decaf-lang/decaf-2019-TestCases/tree/master/S4) 中的所有测例。我们还会检查你的代码是否正确实现了要求，因为什么都不改也能过所有测例。
...
- 实验报告（根目录下 `report-PA5.pdf` 文件）：20%。要求包含以下内容：
    ...
    <!-- 4. 如果你只实现了局部寄存器分配算法，试叙述如何将你的实现改为全局寄存器分配算法(在函数层面)。与局部算法相比，全局算法有什么优势和劣势？ -->


注意：由于 PA5 为选做任务，本阶段**既不**安排 pretest，**也不**接受任何形式的补交。
...
> 在 [PA4 实验指导](PA4.md) 中已有说明，下面这段只是原封不动地搬运过来。
...
Decaf 采用一种很简单的局部寄存器分配策略，在基本块开头，把 liveIn 的变量 load 进寄存器，基本块内为变量分配寄存器，在离开基本块时，将liveOut 的变量 store 出去。局部寄存器分配是对每个基本块分别进行寄存器分配的方法。
...
在 Decaf 中，由于栈桢的大小在编译期的时候是可以计算出来的，因此 $fp 实际上是 不需要的，因为移动和恢复 $sp 的时候，可以简单的通过加减立即数来做到，并不需要事先保存。在实际的编译器里，在栈大小小于 65536 个字节，并且没有 alloca 之类会动态分 配栈空间的操作时，也是不会使用 $fp 的。

### 调用约定

比较详细的调用约定(calling convention)参见[4]。与寄存器相关的调用约定如下：

![](assets/PA5-mips-calling-convention.png)

其中灰色的寄存器为被调用者保存的寄存器，白色的为调用者保存的寄存器。值得注意的是，虽然 $ra 由被调用者保存，它的值也的确在进入和离开被调用的函数时保持一致，但是函数调用指令本身会修改它。

在 Java 和 Scala 版本的框架中，寄存器分配的范围限制在 $v1，$t0~$t9，$s0~$s8 这 20 个寄存器，其他一些寄存器例如 $ra，$v0，$a0~$a3 都有自己特殊的用途，为了简单起见它们不参与寄存器的分配。

对于被调用者保存的寄存器，会在函数的 prologue 部分将它们保存到栈上，而在 epilogue 部分生成代码将它们恢复(只需要保存和恢复实际参与分配的那些)。
...
2. 对干涉图着色；
3. 根据染⾊结果, 确定 Temp 分配到哪个寄存器, 还是 spill 到内存。

⼲涉图是⼀个⽆向图 $G$，节点是 Temp (⼀种理解是将 Temp 认为是⽆穷多的虚拟寄存器)。不过，根据调用约定，有些 Temp 被分配为什么寄存器是确定的，例如 MIPS 中 $v0 用于保存函数返回值，$a0~$a3 用于保存参数，可认为这些节点已经被**预着色**了，建图时无需考虑它们。
