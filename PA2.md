...
你需要理解并修改框架中对抽象语法树 `VarSel` 节点的处理，原来只能处理类的成员变量，你需要将其扩展为支持类的成员方法。当将方法名赋值给一个变量时，方法名的访权限与调用该方法时的情形相同，例如 `var f = A.print;` 没有访问权限错误当且仅当在同一地点访问 `A.print(...);` 时没有权限错误。下面是几个错误的例子：
