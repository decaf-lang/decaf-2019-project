...
关于“找出两版本代码之间的差异，合并到另一处代码”这件事，有许多工具可以自动化完成。首先需要制作一些补丁文件，我们已经提供了“PA2 单独框架相对于 PA1-A 单独框架的差异”与“最新版框架相对于上一版框架的差异”两种补丁文件，而对于“你的代码与 PA1-A 单独框架的差异”，你可以使用 `git diff` 自行制作补丁。然后你可以使用 `git apply` / `patch` 等工具自动完成打补丁操作：
...
> 提示：
>
> 为了后续代码生成的方便，Lambda 表达式也需要有相应的符号。我们建议你对 Lambda 表达式定义一种新的作用域 `LambdaScope` (参考 `MethodScope`)，定义一种新的符号 `LambdaSymbol` (参考 `MethodSymbol`)，以便对 Lambda 表达式具有的属性进行更好的管理。当然我们不会评估你是否真的做了这件事，用自己认为方便的方法实现即可。
>
